...
class editgui(Tk):
    def __init__(self):
        ...
        self.but=Button(self,text="DELETE",font=fontsty2,bg='grey',fg='cyan',width=10,command=self.deleting).place(x=300,y=450)
    ...
    def deleting(self):
       ...
       print(temp)
       qry="delete from corp where shortform ='temp'"
       ack=cur.execute(qry)
       if ack!=0 :
           messagebox.showinfo("INFO","CORPORATE HAS DELETED")
       else:
           messagebox.showinfo("INFO","DELETE CANNOT BE DONE")   
       con.close()    
