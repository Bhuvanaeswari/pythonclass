...
class  directorygui(Tk):
    ...
    def submit(self):
        ...
        qry1=""" insert into directfirst(accountno,accountname) values ('%d','%s')""" %(an,nn)
        qry2=""" insert into directsecond(accountno,bankname,ifsccode,branch) values ('%d','%s','%s','%s')""" %(an,bn,ic,brn)
        cur=con.cursor()
        ack1=cur.execute(qry1)
        ack2=cur.execute(qry2)
        print(ack1,ack2)
        if ack1!=0 and ack2!=0 :
